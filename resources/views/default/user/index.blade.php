@extends($view_prefix . '/layouts/child')
@section('css')
<link href="{{asset_site($resource, 'plugin', 'jqgrid/css/ui.jqgrid.css')}}" rel="stylesheet">
@endsection
...
@section('js')
<script type="text/javascript" src="{{asset_site($resource, 'plugin', 'jqgrid/js/jquery.jqGrid.min.js')}}"></script>
<script type="text/javascript" src="{{asset_site($resource, 'plugin', 'jqgrid/js/i18n/grid.locale-cn.js')}}"></script>
<script type="text/javascript" src="{{asset_site($resource, 'js', 'jqgrid-custom.js')}}"></script>
<script type="text/javascript">
var statusJson = {"1":"正常","0":"禁用"};
var roleJson = $.parseJSON('{{$role}}');
$(document).ready(function() {
	$("#table_list_2").jqGrid({
        url: "{{route('manage.list')}}",
		editurl: "{{route('manage.modify')}}",
		datatype: "json",
